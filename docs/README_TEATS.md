## Del's README on the Teats 

_Some Qs After Everything:_

_1. 2-1/2" means 2+- 1/2")?_
_2. What kind of simplifications for better for R? Like expanding the eq?_
_3. Length of the Teat isn't explicitly measured but can be changed in the eq.__


## General Information/Understanding

Teats are underneath the udders. What's looked at in the Booklet:
- Medial Suspensory Ligament (The cleft/septum looking thing)
- Teat Placement (Relative to Hocks/Knees; Slanted Sliding Scale) 
- Teat Diameter (Wideness)  

## Understanding of the Linear Appraisal System 

Basically 1-50. 

### Teat Placement:

_Position determined by the center of the teat, at the point where it attaches to the udder_

Outside Third (less than 25) < Two-Thirds the Way Out (>25) < One-Third the Way Out (>45)

### 

_Measured at Base_
_They also use this to understand the position of the teat from the udder & ease of milking_

1. 1/2" or less --> 5 or less 
2. 1-1/2" --> 25 or more 
3. 2-1/2" --> 45 or more

_1/2" differences give 10 pt differences_


## Defects? 

56.  Too Short <1/2"?
57.  Too Long > 2-1/2"?
58.  Lack Deliniation???
59.  Abnormal Place
60.  Abnormal Shape 
61.  Extra Teats 
62.  Teats Point Out 
63.  Blind Teats (No Opening)
64.  Double Teat or Orifice (Teat splits to two/Orifice too?) 

## Desmos Equation Understanding 

So far it looks like a Parabolic Equation: 

'f(x) = h(x + j)(x + (q + j)) - (o + u)' 

where: 
- x is a placeholder 
- j is the distance from origin or x = 0, does halve the singular teat when j = 0 
- q is the roundness of the teat, does the sharpness (this is dependent on the medial)
- o is the length of the udder, also affects another equation (medial
	- o and q are discrete and I think independent, like doesn't affect the teats at all really? Just the placement in the graph.
- u is the length of the teat (not mentioned in the rating systems) 

