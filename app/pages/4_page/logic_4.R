# Page 4 logic - the visualizer. This is the original code we've worked on for most of the quarter
# but now its put into a specific page module function

logic4 <- function(input, output, session, rv, S) {

  # +/- / reset / min / max badge wiring for every input in PARAM_DEFAULTS.
  # The +/- buttons nudge by 1; numericInput's own `step` is the keyboard-arrow
  # granularity. local() captures pid so the closures don't all see the last
  ...
  for (param_id in names(PARAM_DEFAULTS)) {
    local({
      ...
      # clamp out-of-range typed values back into the allowed window, capped at
      # 2dp. Only write back when the cleaned value differs so the cursor isn't
      # reset on every valid keystroke.
  ...
  # split validation so the plot only invalidates on adjustable-param changes
  # or a new lock-in (not on every score keystroke).
  adjustable_params_filled <- reactive({
    all(vapply(names(ADJUSTABLE_DEFAULTS),
               function(pid) is_valid_number(input[[pid]]), logical(1)))
  })
  ...
  # with default ignoreInit = FALSE fires once at startup so the plot renders
  ...
  # geometry = locked scores + live adjustable params.
  geometry <- reactive({
    s <- locked_scores()
  ...
  legs_poly <- reactive({
    legs_polygon_df(leg_width = input$leg_width, hock_height = input$hock_height,
                    top_y = view_top, bot_y = view_bottom)
  })
  hocks_poly   <- reactive({ hocks(hock_height = input$hock_height, leg_width = input$leg_width) })
  hock_midline <- reactive({ hockmidline(hock_height = input$hock_height, leg_width = input$leg_width) })
  pelvic_poly  <- reactive({ pelvic_polygon_df(leg_width = input$leg_width, top_y = view_top) })
  ...
  goat_raster <- reactive({
    ...
    img <- magick::image_colorize(img, opacity = 100 - input$img_opacity, color = "white")
    img <- magick::image_rotate(img, input$rotation)
    as.raster(img)
  })

  # shared plot builder so renderPlot and the PNG export stay in sync
  build_plot <- function() {
    g <- ggplot() +
  ...
  output$export_png <- downloadHandler(
    filename = function() sprintf("goat-plot-%s.png", format(Sys.time(), "%Y%m%d-%H%M%S")),
  ...
  # expose readiness so the Add Observation button (and page 5) can react
  S$obs_ready <- reactive({
    adjustable_params_filled() && locked_scores_filled() && !is.null(rv$selected_goat)
  })

  # ---- Add Observation primary button (HCI #5: disabled until ready) ----
  output$add_obs_button <- renderUI({
    if (isTRUE(S$obs_ready())) {
      actionButton("add_observation", "Add Observation →", class = "btn btn-primary")
    } else {
      tags$button("Add Observation →", class = "btn btn-primary", disabled = NA,
                  title = "Select a goat and fill every score first")
    }
  })
}
